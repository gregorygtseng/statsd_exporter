...
var (
	...
	strictMatch         = flag.Bool("statsd.mapping-strict-match", false, "Only store metrics that match the mapping configuration.")
...
func serveHTTP() {
	...
	if *listenTLScert != "" && *listenTLSkey != "" {
		http.ListenAndServeTLS(*listenAddress, *listenTLScert, *listenTLSkey, nil)
	} else {
